...
function offsetsForMode(mode: HarmonyMode, count: number): number[] {
  ...
  switch (mode) {
    ...
    case "triadic":
      ...
      return Array.from(
        { length: count },
        (_, i) => (i * (360 / Math.max(3, count))) % 360,
      ).map((v) => v - 60);
    case "tetradic":
      if (count === 4) return [0, 90, 180, 270];
      return Array.from(
        { length: count },
        (_, i) => (i * (360 / Math.max(4, count))) % 360,
      ).map((v) => v - 45);
    case "monochrome":
...
export function generatePaletteFromBase(opts: {
  ...
  const hexes = Array.from({ length: count }, (_, i) => {
    const hueOff = hueOffsets[i];
    const lightStep = lightnessSteps[i];
    if (hueOff === undefined || lightStep === undefined) {
      throw new Error("palette: hue/light arrays must match count");
    }
    const h = (baseHsl.h + hueOff) % 360;
    const s = clamp(baseHsl.s * satMul, 0.25, 1);
    const l = clamp(lightStep, 0, 1);
