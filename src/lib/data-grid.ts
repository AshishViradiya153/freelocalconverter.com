...
export function getDataGridColumnWidthStyle<TData>(params: {
  column: Column<TData>;
  cssVarName: string;
}): Pick<React.CSSProperties, "width" | "minWidth"> {
  const { column, cssVarName } = params;
  const fixed = `calc(var(${cssVarName}) * 1px)`;
  if (column.id === "select" && !column.getIsPinned()) {
    return {
      width: "max-content",
      minWidth: fixed,
    };
