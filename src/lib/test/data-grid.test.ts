...
describe("getIsInPopover", () => {
  it("treats dialog content as inside popover (blocks grid undo/redo shortcuts)", () => {
    const root = document.createElement("div");
    root.setAttribute("data-slot", "dialog-content");
    const input = document.createElement("input");
    root.appendChild(input);
    document.body.appendChild(root);
    expect(getIsInPopover(input)).toBe(true);
    document.body.removeChild(root);
  });

  it("treats alert-dialog content the same way", () => {
    const root = document.createElement("div");
    root.setAttribute("data-slot", "alert-dialog-content");
    const btn = document.createElement("button");
    root.appendChild(btn);
    document.body.appendChild(root);
    expect(getIsInPopover(btn)).toBe(true);
    document.body.removeChild(root);
  });
});

describe("getDataGridColumnWidthStyle", () => {
  it("uses max-content for unpinned select", () => {
    const column = { id: "select", getIsPinned: () => false as const };
    expect(
      getDataGridColumnWidthStyle({
        column: column as never,
        cssVarName: "--col-select-size",
      }),
    ).toEqual({
      width: "max-content",
      minWidth: "calc(var(--col-select-size) * 1px)",
    });
  });

  it("uses fixed var width for pinned select", () => {
    const column = { id: "select", getIsPinned: () => "left" as const };
    expect(
      getDataGridColumnWidthStyle({
        column: column as never,
        cssVarName: "--header-select-size",
      }),
    ).toEqual({ width: "calc(var(--header-select-size) * 1px)" });
  });

  it("uses fixed var width for non-select columns", () => {
    const column = { id: "name", getIsPinned: () => false as const };
    expect(
      getDataGridColumnWidthStyle({
        column: column as never,
        cssVarName: "--col-name-size",
      }),
    ).toEqual({ width: "calc(var(--col-name-size) * 1px)" });
  });
});
