...
type RowsHistoryVariant =
  | "cells_update"
  | "rows_add"
  | "rows_delete"
  | "row_reorder";

interface RowsHistoryEntry<TData> {
  kind: "rows";
  variant: RowsHistoryVariant;
  count: number;
  timestamp: number;
...
type HistoryEntry<TData> = RowsHistoryEntry<TData> | SessionHistoryEntry;
...
interface ColumnReorderUndoActions {
  undo: () => void;
  redo: () => void;
}
...
interface UseDataGridUndoRedoReturn<TData> {
  ...
  trackRowReorder: (previousIds: string[], nextIds: string[]) => void;
  trackColumnReorder: (actions: ColumnReorderUndoActions) => void;
}
...
function useDataGridUndoRedo<TData>({
  ...
  const onCommit = React.useCallback(() => {
    ...
    const entry: RowsHistoryEntry<TData> = {
      kind: "rows",
  ...
  const trackRowReorder = React.useCallback(
    (previousIds: string[], nextIds: string[]) => {
      if (!propsRef.current.enabled) return;
      if (previousIds.length !== nextIds.length) return;
      if (previousIds.length === 0) return;
      if (previousIds.every((id, i) => id === nextIds[i])) return;
      ...
      const entry: RowsHistoryEntry<TData> = {
        ...
        timestamp: Date.now(),
        undo: (currentData) =>
          reorderRowsByStableIds(currentData, getRowId, previousIds),
        redo: (currentData) =>
          reorderRowsByStableIds(currentData, getRowId, nextIds),
      };
  ...
  const trackColumnReorder = React.useCallback(
    (actions: ColumnReorderUndoActions) => {
      if (!propsRef.current.enabled) return;
