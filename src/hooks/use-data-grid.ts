...
import {
  compileInsensitiveLiteralFind,
  compileInsensitiveLiteralReplace,
} from "@/lib/literal-text-match";
import type {
  CellPosition,
  CellUpdate,
  ContextMenuState,
...
function useDataGrid<TData>({
  ...
  const scrollToRowForSearchRef = React.useRef<
    (opts: Partial<CellPosition>) => Promise<void>
  >(async () => {});
  ...
  const onSearch = React.useCallback(
    (query: string) => {
      ...
      let findRe: RegExp;
      try {
        findRe = compileInsensitiveLiteralFind(query);
      } catch {
        store.batch(() => {
          store.setState("searchMatches", []);
          store.setState("matchIndex", -1);
      ...
      const currentTable = tableRef.current;
      const currentData = propsRef.current.data;
      const preRows = currentTable?.getPrePaginationRowModel().rows ?? [];

      for (let preIndex = 0; preIndex < preRows.length; preIndex++) {
        const row = preRows[preIndex];
        if (!row) continue;

        const dataRowIndex = currentData.indexOf(row.original);
        if (dataRowIndex < 0) continue;

        for (const cell of row.getVisibleCells()) {
          const columnId = cell.column.id;
          if (!searchableColumnIdSet.has(columnId)) continue;
    ...
    [searchableColumnIdSet, store],
  );

  const onSearchReplaceAll = React.useCallback(
    (find: string, replacement: string) => {
      if (!find.trim() || propsRef.current.readOnly) return;

      let replaceRe: RegExp;
      try {
        replaceRe = compileInsensitiveLiteralReplace(find);
      } catch {
        return;
      }
      ...
      for (const row of preRows) {
        ...
        for (const cell of row.getVisibleCells()) {
          ...
          const str = String(cell.getValue() ?? "");
    ...
    [searchableColumnIdSet, onDataUpdate, onSearch, propsRef],
  );
  ...
  const searchMatchSet = React.useMemo(() => {
    ...
    return set;
  }, [searchMatches, paginationEnabled, paginationSearchKey]);
  ...
  const getIsActiveSearchMatch = React.useCallback(
    ...
    [store, paginationEnabled, paginationSearchKey],
  );
  ...
  const tableMeta = React.useMemo<TableMeta<TData>>(() => {
    return {
      ...
      get enableRowReorder() {
        return propsRef.current.enableRowReorder ?? false;
      },
      getIsCellSelected,
      getIsSearchMatch,
  ...
  const tableState = React.useMemo<Partial<TableState>>(
    ...
    [controlledTableState, sorting, columnFilters, rowSelection],
  );
      ...
      if (
        ...
        if (selectedRowIds.length > 0) {
          ...
          for (let i = 0; i < rows.length; i++) {
            const row = rows[i];
            if (row && currentState.rowSelection[row.id]) {
              rowIndices.add(i);
            }
          }
