...
import { kgToLbs } from "@/lib/math/simple-converters";
...
export function KgToLbsConverterApp() {
  const [raw, setRaw] = React.useState("");
  const parsed = React.useMemo(() => parseDecimal(raw), [raw]);

  const output = React.useMemo(() => {
    if (!parsed.ok) return null;
    return formatConverted(kgToLbs(parsed.value));
  }, [parsed]);
  ...
  return (
    <ToolPage>
      <ToolHero
        ...
        description={
          <>
            Convert kilograms (kg) to pounds (lb) locally in your browser.{" "}
        ...
        <ToolPane>
          ...
          <div className="flex flex-wrap items-end gap-2">
            <div className="min-w-48 flex-1">
              <Label className="text-muted-foreground">Pounds (lb)</Label>
