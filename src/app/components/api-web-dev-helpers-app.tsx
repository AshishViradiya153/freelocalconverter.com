...
function formatJsObjectLiteral(
...
function parseCurl(command: string): HttpRequestModel {
  ...
  if (!raw.includes("curl"))
    throw new Error("That doesn't look like a cURL command.");
...
function renderFetch(m: HttpRequestModel): string {
  ...
  if (m.body) {
    parts.push(
      `  body: ${m.body.startsWith("JSON.stringify") ? m.body : jsStringLiteral(m.body)},`,
    );
  }
  parts.push(`});`);
  return parts.join("\n");
...
function renderAxios(m: HttpRequestModel): string {
  ...
  if (m.body) {
    parts.push(
      `  data: ${m.body.startsWith("{") || m.body.startsWith("[") ? m.body : jsStringLiteral(m.body)},`,
    );
  }
  parts.push("});");
  return parts.join("\n");
...
function explainStatus(code: number): {
  title: string;
  meaning: string;
  tips: string[];
} {
  const common: Record<
    number,
    { title: string; meaning: string; tips: string[] }
  > = {
    200: {
      title: "OK",
    ...
    400: {
      title: "Bad Request",
      meaning:
        "Client error: invalid payload, parameters, or validation failure.",
    ...
    409: {
      title: "Conflict",
      meaning:
        "Request conflicts with current state (duplicate, version conflict).",
...
function headerHelp(name: string): { summary: string; notes: string[] } | null {
  ...
  const map: Record<string, { summary: string; notes: string[] }> = {
    "content-type": {
      summary:
        "Declares the media type of the body you are sending or receiving.",
      notes: [
        "Common: application/json, text/plain; charset=utf-8, multipart/form-data.",
      ],
    },
    accept: {
  ...
  return (
    <div className={cn("flex min-h-0 flex-col gap-2", className)}>
      <div className="flex items-center justify-between gap-3">
        <ToolPaneTitle className="text-[11px]">{title}</ToolPaneTitle>
        {readOnly ? (
          <Button
            type="button"
            variant="outline"
      ...
      <Textarea
        ...
        readOnly={readOnly}
        className={cn(
          toolEditorClassName("min-h-56 leading-relaxed"),
...
export function ApiWebDevHelpersApp({
  ...
  const [fromFmt, setFromFmt] =
    React.useState<ConverterFormat>(initialFromFormat);
  ...
  const [openapiFileName, setOpenapiFileName] = React.useState<string | null>(
    null,
  );
  const [openapiSelectedPath, setOpenapiSelectedPath] = React.useState<
    string | null
  >(null);
  const [openapiSelectedMethod, setOpenapiSelectedMethod] = React.useState<
    string | null
  >(null);
  ...
  const openapiParsed = React.useMemo(() => {
    ...
    try {
      ...
      return {
        ok: true as const,
        error: null as string | null,
        doc: { doc, version, paths, servers, components },
      };
    } catch (error) {
      return {
        ok: false as const,
  ...
  const openapiPathList = React.useMemo(() => {
    ...
    return Object.keys(openapiParsed.doc.paths).sort((a, b) =>
      a.localeCompare(b),
    );
  ...
  const [gqlSchemaFileName, setGqlSchemaFileName] = React.useState<
    string | null
  >(null);
  const [gqlTypeFilter, setGqlTypeFilter] = React.useState<string>("");
  const [gqlTypeSelected, setGqlTypeSelected] = React.useState<string | null>(
    null,
  );

  const gqlSchema = React.useMemo(() => {
    if (!gqlSchemaText.trim())
      return {
        ok: false as const,
        error: null as string | null,
        schema: null,
  ...
  return (
    <ToolPage>
      <ToolHero title={title} description={subtitle} />

      {showToolSwitcher ? (
        <ToolCard className="space-y-4">
          <ToolToolbar className="gap-3">
            <ToggleGroup
              type="single"
              value={tool}
              onValueChange={(v) => {
      ...
      {tool === "converter" ? (
        <ToolCard className="grid grid-cols-1 gap-6 lg:grid-cols-2">
          <ToolPane>
            <ToolToolbar className="gap-3">
              <div className="grid gap-1">
                ...
                >
                  ...
                  <ToggleGroupItem value="python-requests">
                    Python
                  </ToggleGroupItem>
                </ToggleGroup>
              </div>
          ...
          <ToolPane>
            <div className="flex items-center justify-between gap-3">
              <div className="text-muted-foreground text-sm">
                {converterResult.ok
                  ? "Parsed request and rendered output."
            ...
            <CodePane
              title="Output"
              value={converterResult.ok ? converterResult.text : ""}
              readOnly
            />
            <div className="rounded-lg border bg-muted/10 p-3 text-muted-foreground text-xs leading-relaxed">
              Conversions are best-effort for common snippet shapes (headers +
              method + body). If your snippet uses advanced features (multipart,
              cookie jars, interceptors), the output may need small manual
              tweaks.
            </div>
          </ToolPane>
        </ToolCard>
                  ...
                  {httpExplainer.explained.map((h) => (
                    ...
                    >
                      ...
                      <div className="mt-2 text-muted-foreground text-sm leading-relaxed">
                        {h.help?.summary ??
                          "No built-in notes for this header yet."}
                      </div>
                      {h.help?.notes?.length ? (
      ...
      {tool === "openapi" ? (
        <ToolCard className="grid grid-cols-1 gap-6 lg:grid-cols-[440px_1fr]">
          <ToolPane className="gap-4">
            <FileDropZone
              disabled={false}
              busy={false}
          ...
          <div className="min-h-0 rounded-xl border bg-muted/5">
            {!openapiParsed.ok ? (
              ...
              <div className="grid min-h-0 grid-cols-1 lg:grid-cols-[320px_1fr]">
                <div className="min-h-0 border-border border-b lg:border-r lg:border-b-0">
                  <div className="p-4">
                    <div className="flex items-baseline justify-between gap-3">
                      <div className="font-medium text-sm">Paths</div>
                      <div className="text-muted-foreground text-xs">
                        {openapiParsed.doc.version
                          ? `v${openapiParsed.doc.version}`
                          : "OpenAPI"}
                      </div>
                    </div>
                  </div>
                    ...
                    <div className="min-h-0">
                      <div className="flex flex-wrap items-center justify-between gap-3">
                        ...
                        <ToggleGroup
                          ...
                          onValueChange={(v) =>
                            v && setOpenapiSelectedMethod(v)
                          }
                          variant="outline"
                          size="sm"
                        >
                          {Object.keys(
                            openapiParsed.doc.paths[openapiOp.path] ?? {},
                          )
                            .filter(
                              (k) =>
                      ...
                      <div className="mt-4 grid gap-4">
                        <div className="rounded-lg border bg-background p-4">
                          ...
                          {Array.isArray(openapiOp.op.tags) &&
                          openapiOp.op.tags.length ? (
      ...
      {tool === "graphql" ? (
        <ToolCard className="grid grid-cols-1 gap-6 lg:grid-cols-2">
          ...
          <ToolPane className="min-h-0 gap-4">
            <FileDropZone
              disabled={false}
              busy={false}
            ...
            <ToolToolbar>
              <Button
                ...
                onClick={() => {
                  if (!webhookParsed.ok) {
                    toast.error(
                      webhookParsed.error ?? "Paste valid JSON first.",
                    );
                    return;
                  }
          ...
          <ToolPane className="min-h-0 gap-4">
            <div className="rounded-xl border bg-muted/10 p-4">
              ...
              <div className="mt-3">
                {!webhookParsed.ok ? (
                  <div className="text-muted-foreground text-sm">
                    {webhookParsed.error
                      ? `Parse error: ${webhookParsed.error}`
                      : "Paste JSON to search."}
                  </div>
                ) : webhookMatches.ok ? (
                  ...
                  <div className="text-destructive text-sm">
                    JSONPath error: {webhookMatches.error}
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="rounded-xl border bg-muted/10 p-4">
              ...
              <pre className="mt-2 max-h-[45vh] overflow-auto whitespace-pre-wrap rounded-lg border bg-background p-3 font-mono text-xs leading-relaxed">
                {webhookParsed.ok
                  ? webhookParsed.pretty
                  : "Paste JSON to render a pretty view."}
              </pre>
            </div>
          </ToolPane>
        </ToolCard>
