...
export function ParquetToJsonApp() {
  const t = useTranslations("parquetToJson");
  ...
  const previewKey = React.useMemo(() => {
    if (!session) return "";
    return `parquet-to-json-${session.rows.length}-${session.columnKeys.join(",")}`;
  }, [session]);
