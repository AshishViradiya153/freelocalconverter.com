  ...
  async function onSplit() {
    ...
    try {
      ...
      const ranges = selection.trim()
        ? parsePdfPageSelection(selection, pageCount).map(
      ...
      if (outputs.length === 1) {
        ...
        const suffix = selection.trim()
          ? outputs[0]?.name.replace(/^\d+-/, "")
          : "page-1.pdf";
      ...
      if (msg === "empty_file") setError(`"${file.name}" is empty (0 bytes).`);
        ...
        setError(`"${file.name}" has no pages to split.`);
    ...
    <div className="container flex flex-col gap-6 py-4">
      <header className="flex flex-col gap-1">
        <div className="flex items-center gap-2">
          ...
          <h1 className={toolHeroTitleClassName}>Split PDF</h1>
        </div>
        <p className="max-w-3xl text-muted-foreground text-sm">
          Split one PDF into multiple PDFs locally in your browser, no uploads.
          Export each page as its own PDF, or choose ranges.
        </p>
      </header>
