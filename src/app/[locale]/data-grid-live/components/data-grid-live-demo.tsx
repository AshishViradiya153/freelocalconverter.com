...
export function DataGridLiveDemo() {
  ...
  > = React.useCallback(async (_args: { fileIds: string[] }) => {
    // Local demo: files use blob URLs only; nothing remote to delete
  }, []);

  const { table, tableMeta, searchState, ...dataGridProps } = useDataGrid({
    data,
    onDataChange,
    onRowAdd,
    onRowsAdd,
