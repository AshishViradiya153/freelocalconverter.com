...
function matchesColor(p: BestPaletteRow, colorId: ColorId) {
  if (colorId === "all") return true;
  return p.colorTag === colorId;
}

function matchesStyle(p: BestPaletteRow, styleId: StyleId) {
  if (styleId === "all") return true;

  if (styleId === "classic") return p.styleClassic;
  if (styleId === "dark-modern") return p.styleDarkModern;
  if (styleId === "warm") return p.styleWarm;
  if (styleId === "cold") return p.styleCold;
  if (styleId === "bright") return p.styleBright;
  if (styleId === "dark") return p.styleDark;
  if (styleId === "pastel") return p.stylePastel;
  if (styleId === "vintage") return p.styleVintage;
  if (styleId === "monochromatic") return p.styleMonochromatic;
  if (styleId === "gradient") return p.styleGradient;

  return true;
}

export default function BestPalettesGrid({
  palettes,
  locale: _locale,
}: {
  palettes: BestPaletteRow[];
  locale: string;
}) {
  ...
  const filtered = React.useMemo(() => {
    const next = palettes.filter((p) => {
      if (!matchesColor(p, colorId)) return false;
      if (!matchesStyle(p, styleId)) return false;
      return true;
    });
    ...
    return next;
  }, [palettes, sortId, styleId, colorId]);
      ...
      {palettes.length === 0 ? (
        ...
        <>
          <div className="grid grid-cols-1 gap-0 sm:grid-cols-2 lg:grid-cols-3">
            {visible.map((row) => (
              <BestPaletteCard key={row.idx} row={row} locale={_locale} />
            ))}
          </div>
