...
export async function generateMetadata({
  params,
}: MergeCsvPageProps): Promise<Metadata> {
  const { locale } = await params;
  return await buildToolPageMetadata(locale, "merge-csv");
}

interface MergeCsvPageProps {
  params: Promise<{ locale: string }>;
}

export default async function MergeCsvPage({ params }: MergeCsvPageProps) {
  const { locale } = await params;
