...
export async function generateMetadata({
  params,
}: OnlineMsgViewerPageProps): Promise<Metadata> {
  const { locale } = await params;
  return await buildToolPageMetadata(locale, "online-msg-viewer");
}

interface OnlineMsgViewerPageProps {
  params: Promise<{ locale: string }>;
}

export default async function OnlineMsgViewerPage({
  params,
}: OnlineMsgViewerPageProps) {
  const { locale } = await params;
