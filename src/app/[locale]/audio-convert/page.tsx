...
export async function generateMetadata({
  params,
}: AudioConvertPageProps): Promise<Metadata> {
  const { locale } = await params;
  return await buildToolPageMetadata(locale, "audio-convert");
}
...
export default async function AudioConvertPage({ params }: AudioConvertPageProps) {
  const { locale } = await params;
