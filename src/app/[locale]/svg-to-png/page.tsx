...
import { SvgToPngApp } from "@/app/components/svg-to-png-app";
import { Shell } from "@/components/shell";
import { buildToolPageMetadata } from "@/lib/seo/tool-page-metadata";

export async function generateMetadata({
  params,
}: SvgToPngPageProps): Promise<Metadata> {
  const { locale } = await params;
  return await buildToolPageMetadata(locale, "svg-to-png");
}
...
export default async function SvgToPngPage({ params }: SvgToPngPageProps) {
  const { locale } = await params;
