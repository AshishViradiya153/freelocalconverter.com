...
export const DataGridCell = React.memo(DataGridCellImpl, (prev, next) => {
  ...
  const prevCompare =
    typeof prev.tableMeta?.getCompareDiffHighlight === "function"
      ? prev.tableMeta.getCompareDiffHighlight(
          String(prev.cell.row.id),
          prev.columnId,
        )
      : false;
  const nextCompare =
    typeof next.tableMeta?.getCompareDiffHighlight === "function"
      ? next.tableMeta.getCompareDiffHighlight(
          String(next.cell.row.id),
          next.columnId,
        )
      : false;
  if (prevCompare !== nextCompare) return false;
...
function DataGridCellImpl<TData>({
  ...
  const compareDiffHighlight =
    typeof tableMeta?.getCompareDiffHighlight === "function" &&
    tableMeta.getCompareDiffHighlight(String(cell.row.id), columnId);

  return (
    <div
      className={cn(
        "size-full min-h-0 min-w-0",
        compareDiffHighlight &&
          "**:data-[slot=grid-cell-wrapper]:bg-amber-500/20 **:data-[slot=grid-cell-wrapper]:dark:bg-amber-950/60",
      )}
    >
      <Comp
        cell={cell}
