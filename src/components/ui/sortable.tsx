...
function SortableRoot<T>(props: SortableRootProps<T>) {
  ...
  const sensors = useSensors(
    useSensor(MouseSensor, {
      activationConstraint:
        mouseActivationDistance != null && mouseActivationDistance > 0
  ...
  const onDragEnd = React.useCallback(
    (event: DragEndEvent) => {
      ...
      if (resolveOrderOnDragEnd) {
        const resolved = resolveOrderOnDragEnd(event, value);
        if (resolved !== undefined) {
          const changed =
            resolved.length !== value.length ||
            resolved.some(
              (item, index) =>
                getItemValue(item) !== getItemValue(value[index] as T),
            );
          if (changed) {
            if (onMove) {
              ...
              const newIndex = resolved.findIndex(
                (item) => getItemValue(item) === active.id,
              );
              if (activeIndex !== -1 && newIndex !== -1) {
                onMove({ ...event, activeIndex, overIndex: newIndex });
              }
            } else {
      ...
      if (over && active.id !== over?.id) {
        ...
        if (onMove) {
          ...
          onValueChange?.(moveArrayElement(value, activeIndex, overIndex));
        }
      }
      setActiveId(null);
    },
...
interface SortableItemProps extends React.ComponentProps<"div"> {
  ...
  asChild?: boolean;
  disabled?: boolean;
  /** Disable sibling layout animations during drag (helps virtualized lists). */
...
function SortableItem(props: SortableItemProps) {
  const {
    ...
    asChild,
    disabled,
    animateLayoutChanges: animateLayoutChangesProp,
    className,
    ref,
    ...itemProps
  } = props;
