/**
 * Ensures every messages/*.json has the same `pageMeta` leaf keys as en.json.
 * Run after editing pageMeta: `node scripts/validate-page-meta-locales.mjs`
 */
...
/** Dot-notation keys for string leaves only */
...
const expected = leafKeys(en.pageMeta).sort();
if (expected.length === 0) {
  console.error("en.json: pageMeta has no string leaves");
  process.exit(1);
}
...
for (const name of fs.readdirSync(messagesDir)) {
  ...
  if (!data.pageMeta) {
    console.error(`${name}: missing pageMeta`);
    failed = true;
    continue;
  }
  const actual = leafKeys(data.pageMeta).sort();
...
console.log(
  `pageMeta OK: ${expected.length} keys × ${fs.readdirSync(messagesDir).filter((f) => f.endsWith(".json")).length} locales`,
);
